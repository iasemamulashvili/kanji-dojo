...
export default async function PracticePage({ searchParams }: PracticePageProps) {
  ...
  try {
    if (isQuizMode) {
      // Quiz Mode: fetch a random kanji, bypassing group_settings
      const { count, error: countError } = await supabase
        .from('kanjis')
        .select('*', { count: 'exact', head: true });

      if (countError || !count || count === 0) {
        errorMsg = "No Kanji available for quiz.";
      } else {
        const randomOffset = Math.floor(Math.random() * count);
        const { data: randomKanji, error: randomError } = await supabase
          .from('kanjis')
          .select('*')
        ...
        if (randomError) {
          console.error("Random Quiz Query Error:", randomError);
          errorMsg = "Failed to load a random Kanji for quiz.";
        } else {
          kanjiData = randomKanji;
        }
      }
    } else {
  ...
  return <PracticeClient kanjiData={kanjiData} isQuizMode={isQuizMode} />;
}
