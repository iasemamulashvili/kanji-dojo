...
function OrganicProgress({ value, label, icon: Icon, colorClass = "text-accent" }: { value: number, label: string, icon: any, colorClass?: string }) {
  const radius = 35;
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-24 h-24 flex items-center justify-center">
        ...
        <div className="z-10 flex flex-col items-center">
           ...
           <span className="text-xl font-black text-text-main">{Math.round(value)}%</span>
        </div>
      </div>
      <span className="mt-3 text-[0.65rem] font-bold tracking-widest uppercase text-text-muted/60">{label}</span>
    </div>
  );
}
...
export default function StatsPage() {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="wabi-card p-12 flex flex-col items-center animate-pulse">
          ...
          <p className="text-xs font-bold tracking-[0.2em] uppercase text-text-muted">Whispering to the Dojo...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden p-4 md:p-8 flex flex-col items-center bg-background text-text-main">
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col items-center">
        ...
        <header className="flex flex-col items-center w-full mb-10">
          <h1 className="text-lg font-bold tracking-[0.3em] uppercase mb-1 text-text-main">Mastery Scroll</h1>
          <div className="text-[0.6rem] font-bold tracking-widest text-accent uppercase bg-accent/10 px-3 py-1 rounded-full imperfect-border">
        ...
        {/* Filters */}
        <div className="flex flex-col sm:flex-row gap-4 mb-8 w-full justify-between items-center bg-text-muted/5 p-4 rounded-xl border border-text-muted/10">
          <div className="flex gap-2 bg-background p-1 rounded-lg">
            <button 
              onClick={() => setTimeFilter('all-time')}
              className={`text-[0.65rem] font-bold uppercase tracking-widest px-4 py-2 transition-all rounded-md ${timeFilter === 'all-time' ? 'bg-text-muted text-background shadow-sm' : 'text-text-muted/70 hover:text-text-muted'}`}
            >
              All Time
            </button>
            <button 
              onClick={() => setTimeFilter('latest')}
              className={`text-[0.65rem] font-bold uppercase tracking-widest px-4 py-2 transition-all rounded-md ${timeFilter === 'latest' ? 'bg-text-muted text-background shadow-sm' : 'text-text-muted/70 hover:text-text-muted'}`}
            >
              Latest Quiz
            </button>
            <button 
              onClick={() => setTimeFilter('history')}
              className={`text-[0.65rem] font-bold uppercase tracking-widest px-4 py-2 transition-all rounded-md ${timeFilter === 'history' ? 'bg-text-muted text-background shadow-sm' : 'text-text-muted/70 hover:text-text-muted'}`}
            >
              History
            </button>
          </div>
          ...
          {(timeFilter === 'all-time' || timeFilter === 'latest') && (
            <div className="relative">
              <select 
                ...
                className="appearance-none bg-background text-text-muted text-xs font-bold uppercase tracking-widest px-4 py-2 pr-8 rounded-lg border border-text-muted/20 outline-none cursor-pointer"
              >
              ...
              <Filter className="w-3 h-3 text-text-muted absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            </div>
          )}
        </div>
              ...
              if (timeFilter === 'latest') {
                ...
                if (latest?.type_stats) {
                  displayData = latest.type_stats;
                   ...
                   return (
                     <motion.div key="latest-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="col-span-full py-12 text-center text-xs font-medium text-text-muted/60 italic">
              ...
              >
                {stats?.recent.length > 0 ? (
                  stats.recent.map((quiz: any, idx: number) => (
                    <div key={idx} className="wabi-card p-4 flex justify-between items-center group hover:border-text-muted transition-colors cursor-default">
                      <div className="flex items-center gap-4">
                         <div className="w-10 h-10 rounded-full bg-background border border-text-muted/20 flex items-center justify-center font-bold text-xs">
                           {quiz.score}
                         </div>
                         <div className="flex flex-col">
                           ...
                           <span className="text-[0.6rem] text-text-muted/60 uppercase font-bold">
                             {Math.round((quiz.correct_answers / quiz.total_questions) * 100)}% Accuracy
                           </span>
                         </div>
                      </div>
                      <ChevronRight className="w-4 h-4 text-text-muted opacity-20 group-hover:opacity-100 transition-all translate-x-0 group-hover:translate-x-1" />
                    </div>
                  ))
                ) : (
        ...
        <div className="fixed bottom-0 left-0 w-full p-4 flex justify-center bg-gradient-to-t from-parchment via-parchment to-transparent pt-12 z-20 pointer-events-none">
           <p className="text-[0.6rem] font-bold tracking-[0.4em] uppercase text-text-muted/40 mb-4 select-none">
